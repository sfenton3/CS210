...
public class AVLLinkedBinarySearchTree<T extends Comparable<T>>
{
   ...
   ******************************************************************/
   ...
   {
     if(node.balanceFactor == -2){//Focus on Left children
       if(node.left.balanceFactor == 1){ //LeftRight Rotation
         rotateLeft(node.left); //pivot around child node
         rotateRight(node);
         System.out.println("Perform LeftRight rotation around: " + node);
       }
       else{ //Right rotation at -1 or 0
         System.out.println("Perform Right rotation around: " + node);
         rotateRight(node); }     
       return true;
     }
     if(node.balanceFactor == 2){ //Focus on Right children
       if(node.right.balanceFactor == -1){ //RightLeft Rotation
         rotateRight(node.right); //pivot around child node
         rotateLeft(node);
         System.out.println("Perform RightLeft rotation around: " + node);
       }
       else{
         rotateLeft(node);//Left Rotation at -1 or 0
    ...
    //return true;   // stub to allow it to compile
   ...
   private void rotateLeft(AVLBinarySearchTreeNode<T> pivot)
   {
     ...
     pivot.balanceFactor = (height(pivot.right) - height(pivot.left)); //balance factor of pivot is right minus left
     newPivot.balanceFactor = (height(newPivot.right) - height(newPivot.left)); //balance factor of newPivot is right minus left
   }
 
   
    //Used to find the height of 1 side of a tree recursivly
   public int height(AVLBinarySearchTreeNode<T>  node){
       if(node == null)
         return 0;
       return 
